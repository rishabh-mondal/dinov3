...
- Train (optional), validate, and evaluate IN-REGION + OOR with one-time region variables
...
# ---- Region <-> data roots ----
REGION_ROOTS = {
    "uttar_pradesh": UP_ROOT,
    "bangladesh":    BD_ROOT,
    "pak_punjab":    PKP_ROOT,
}

# ---- Pretty names for titles/logs ----
PRETTY = {
...
# ========= Choose regions ONCE here =========
TRAIN_REGION   = "pak_punjab"                 # used if DO_TRAIN=True
IN_REGION      = "pak_punjab"              # evaluate as IN-REGION
OOR_REGIONS    = ["bangladesh", "uttar_pradesh"] # evaluate as OOR (order matters)
# ============================================
...
DO_TRAIN      = False  # flip to True to train

# derive run tag & io paths from chosen region(s)
RUN_TAG     = TRAIN_REGION if DO_TRAIN else IN_REGION
BEST_CKPT   = f"best_film_val_map50_{RUN_TAG}.pth"
RESULTS_CSV = f"region_eval_film_{RUN_TAG}.csv"
LOG_DIR     = f"runs/brickkiln_dinov3_film_{RUN_TAG}"
...
def evaluate_region(model, region_key: str, split: str, device,
                    batch_size=16, num_workers=8, image_size=224, title_prefix="",
                    results_csv=None):
    ds = BrickKilnDataset(
        root=REGION_ROOTS[region_key], split=split, input_size=image_size,
        region_name=region_key, aef_path=AEF_PATHS[region_key]
    )
    ...
    for batch in tqdm(dl, desc=f"Testing [{PRETTY.get(region_key, region_key)}]"):
        if batch is None:
            continue
        images, targets, conds = batch
    ...
    title = f"{PRETTY.get(region_key, region_key)} — {title_prefix}"
    print("\n" + "=" * 84)
...
def main():
    ...
    if DO_TRAIN:
        train_ds = BrickKilnDataset(
            root=REGION_ROOTS[TRAIN_REGION], split="train", input_size=IMAGE_SIZE,
            region_name=TRAIN_REGION, aef_path=AEF_PATHS[TRAIN_REGION]
        )
        val_ds = BrickKilnDataset(
            root=REGION_ROOTS[TRAIN_REGION], split="val", input_size=IMAGE_SIZE,
            region_name=TRAIN_REGION, aef_path=AEF_PATHS[TRAIN_REGION]
        )
    ...
    evaluate_region(
        model,
        region_key=IN_REGION, split="test", device=device,
        batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, image_size=IMAGE_SIZE,
        title_prefix="IN-REGION (test)",
        results_csv=RESULTS_CSV
    )

    # OOR regions
    for r in OOR_REGIONS:
        evaluate_region(
            model,
            region_key=r, split="test", device=device,
            batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, image_size=IMAGE_SIZE,
            title_prefix="OOR (test)",
            results_csv=RESULTS_CSV
        )
...
# Example:
# CUDA_VISIBLE_DEVICES=2 nohup python -u dinov3_film_regions.py > film_train_pak_punjab.log 2>&1 &